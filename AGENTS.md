...
- **Deque's "Creating Accessible SVGs"** (https://www.deque.com/blog/creating-accessible-svgs/) — guidance on choosing the right accessibility pattern based on SVG deployment context (inline, `<img>`, `<object>`, CSS background), and handling decorative vs informative SVGs.
- **Smashing Magazine's "Accessible SVGs: Inclusiveness Beyond Patterns"** (https://www.smashingmagazine.com/2020/03/accessible-svgs-inclusiveness-beyond-patterns/) — context-dependent accessibility decisions, animated/interactive SVG, `prefers-reduced-motion`, and long-description strategies.
- **Deque University's svg-img-alt axe rule** (https://dequeuniversity.com/rules/axe/4.11/svg-img-alt) — authoritative definition of the axe rule that SVGs with `role="img"` must have a text alternative; used to validate exports.
- **Accesify's "SVG Accessibility: Icons, Graphics & Screen Reader Friendly"** (https://www.accesify.io/blog/svg-accessibility-icons-graphics-screen-reader-friendly/) — icon-specific accessibility patterns and `<use>` sprite accessibility.
- **iamvector's "9 Ways to Enhance Accessibility in SVG Icon Design"** (https://iamvector.com/blog/9-ways-to-enhance-accessibility-in-svg-icon-design/) — a concise checklist of SVG accessibility enhancements.
- **dev.to/accessibly_speaking's "How to Make SVGs Accessible: A Short Guide"** (https://dev.to/accessibly_speaking/how-to-make-svgs-accessible-a-short-guide-1ope) — introductory reference covering title, desc, and ARIA roles.
...
- **MERMAID_ACCESSIBILITY_BEST_PRACTICES.md** — normative accessibility specification for authoring, annotation, linting, and the "Generate prompt to improve this diagram" workflow.
- **MERMAID_TRANSFORMATION_BEST_PRACTICES.md** — normative transformation specification governing post-processing rules, semantic preservation, and verification checklists.
